...
public abstract class Entity<TId> : IEquatable<Entity<TId>> where TId : notnull
{
    ...
    /// <summary>
    /// 採番前のエンティティを生成するコンストラクタ
    /// </summary>
    /// <remarks>
    /// 識別子をデータベースの採番に委ねるエンティティのために用意する。
    /// 注文明細のように、新規に生成した時点では識別子が決まっていないものがある。
    /// この状態のエンティティは<see cref="IsPersisted"/>がfalseになる。
    ///
    /// 管理サービス側にはこのコンストラクタがない。
    /// 管理側は注文を読み取るだけであり、識別子は常にデータベースから得られるため、
    /// 採番前の状態を表現する必要がなかった。
    /// </remarks>
    protected Entity()
    {
